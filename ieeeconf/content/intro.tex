...
\begin{figure}[t]
    ...
    \caption{\textbf{Overview.}
        (Top) Existing approaches provide the policy with the current visual state, leaving the downstream controller to infer task-relevant changes.
...
Predictive representation learning can model these temporal changes, and future prediction helps learn dynamics that are useful for decision making~\cite{hafner2019dream,finn2017deep}. In vision-based representation learning, many methods train by reconstructing or predicting future visual content~\cite{he2022masked,gupta2023siamese,zhu2025lava,zeng2024learning,karamcheti2023language,kim2025token}. However, such objectives may focus on visual details that are irrelevant to manipulation and do not explicitly capture a control-oriented change signal to the policy. Moreover, the temporal component is discarded or compressed after pre-training, i.e. predictive decoders may be removed before downstream learning~\cite{radosavovic2023real,xiao2022masked}, or temporal information remains implicitly in the learned representations. The challenge is therefore not how to learn future prediction, but how to retain the task-relevant part of prediction as an explicit policy input.
