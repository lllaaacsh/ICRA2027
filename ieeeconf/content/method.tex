...
\caption{\textbf{TRAIL architecture in pre-training}. Visual and language tokens are first fused by bidirectional cross-attention. The same predictor is then evaluated under two memory-access conditions: the full-information stream reads class, patch, and language tokens, whereas the intent stream reads only the class and language tokens. Both streams predict the frozen future target, and the full-information prediction provides a stopped consistency target for the intent prediction. The modules with snowflakes mark are frozen, and all other modules are set to trainable. At deployment, only the intent stream is retained to produce the predicted future latent $\hat z^{\mathrm{int}}_{t+k}$ for the downstream policy.}
\label{fig:ab_probe}
\end{figure*}
...
and $L'$, we then perform asymmetric future prediction. The intent stream predicts the full future token sequence $z^{\mathrm{gt}}_{t+k}$ from only the instruction-conditioned class token $V'_{\texttt{cls}}$ and language tokens $L'$, without direct access to current patch tokens $V'_{\texttt{patch}}$. Because this restriction removes much current spatial information,
...
Finally, we optimize both streams with full-prediction, intent-prediction, and consistency losses:
%
\begin{equation}
