...
Trail learns a task-relevant latent change predictor from action-free human videos and uses the predicted change as policy context. During pre-training, a language-grounded encoder and an asymmetric predictor learn to forecast frozen DINOv3 future tokens from Kinetics videos without robot actions; during downstream imitation learning, the retained intent stream produces a latent residual, which is scale-normalised before conditioning the policy. Fig.~\ref{fig:ab_probe} summarizes the architecture.
