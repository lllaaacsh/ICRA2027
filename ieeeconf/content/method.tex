...
TRAIL can be integrated with different downstream policy architectures through the same residual representation. For simulation benchmarks, e.g., Franka Kitchen and Meta-World, we concatenate $z_t$ and $\Delta_z$ with proprioception and use them as input to an action decoder that outputs continuous control commands. For real-robot tabletop tasks, we reshape patch tokens from both $z_t$ and $\Delta_z$ into spatial feature maps and fuse them to predict language-conditioned affordance maps~\cite{shridhar2022cliport}. Thus, the same pre-trained TRAIL representations can be used across different action spaces by adapting only the downstream readout.
