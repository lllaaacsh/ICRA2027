...
TRAIL learns task-relevant latent changes from action-free human videos and uses the predicted change as policy context. During pre-training, a language-grounded encoder and an asymmetric predictor learn to forecast future visual representations under different level of visual access (Sec.~\ref{subsec:pretrain}). During downstream imitation learning, the intent stream produces a latent residual that conditions the policy on task-relevant change (Sec.~\ref{subsec:policy_learning}). 
...
and $L'$, we then perform asymmetric future prediction. The intent stream predicts the full future token sequence $z^{\mathrm{gt}}_{t+k}$ using only the instruction-conditioned class token $V'_{\texttt{cls}}$ and language tokens $L'$, without direct access to the current patch tokens $V'_{\texttt{patch}}$. Since this restricted input removes much of the current spatial information, we additionally introduce a full-information stream during pre-training that receives $V'{\texttt{patch}}$. Its prediction which conditioned on the complete current visual state serves as a consistency target for the intent stream.  The two streams share the predictor $P$ and future queries $Q$, differing only in their memory inputs:
%
\begin{align}
\hat z^{\mathrm{full}}_{t+k}
