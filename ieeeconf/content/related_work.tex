...
Despite these advances, standard latent predictive models (e.g., Siamese MAE~\cite{gupta2023siamese}) still attempt to forecast the \textit{full} future state, inevitably entangling controllable transitions with uncontrollable passive dynamics. In contrast, our proposed TRAIL combines the efficiency of latent prediction with a task-relevant bottleneck. Unlike prior methods, we explicitly produce the \textit{residual} between current and future states, providing policies with a gradient-like directional cue that isolates task-relevant dynamics.

\CO{A related way to use action-free video is to \emph{invent} an action space: latent action
models infer an action variable $u_t$ from an observation pair through a low-capacity bottleneck,
and then align $u_t$ with real commands using a small action-labelled
set~\cite{schmidt2024learning, bruce2024genie, ye2025latent, agibot2025world}. TRAIL shares the
premise but constrains \emph{visual memory access} rather than an action variable, so $\Delta_z$
remains a direction in a frozen perceptual space shared by human and robot, requiring no
latent-to-real alignment stage. In exchange, $\Delta_z$ is not an action and cannot be rolled out
for planning, which makes the two lines complementary. \textbf{XXX REWRITE. THIS PARAGRAPH IS CLEARLY AWKWARD!!! WE HAVE NEVER USED NOTATIONS IN SECTION II. USING LLM IS FINE BUT YOU NEED TO IMPROVE WHAT IS RIGHT TO PUT DOWN. ALSO DON'T SIMPLY COPY PASTE SENTENCES WITH UNNATURAL LINE BREAK, WHICH CLEARLY SHOW YOU HAVEN'T CHECKED THE LATEX. XXX}}
