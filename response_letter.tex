...
\R{Thanks for the suggestion. We revised Sec.~II-B added a discussion on latent action models and compared with TRAIL. Both latent action models and TRAIL are similar in that  both learn dynamics from action-free videos by predicting the future. However, latent action models infer a low-capacity latent action from the current and future frames and reconstruct the future frame conditioned on the current frame and then use the latent action to pre-train policies and must later be mapped to executable actions with action-labelled data. TRAIL instead predicts future representations in latent space through a task-relevant bottleneck on the predictor's visual input, and gives the predicted residual to the policy as an input. Hence, TRAIL avoids pixel-level reconstruction and needs no stage that maps latent actions to executable actions.}
