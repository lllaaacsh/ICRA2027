...
\R{Thanks for the suggestion. Sec.~II-B now positions TRAIL with respect to latent action models, with the additions marked in red. \emph{Similarity:} both learn dynamics from action-free videos by predicting the future. \emph{Difference:} latent action models infer a low-capacity latent action from the current and future frames and reconstruct the future frame conditioned on it; the latent action is used to pre-train policies and must later be mapped to executable actions with action-labelled data. TRAIL instead predicts future representations in latent space through a task-relevant bottleneck on the predictor's visual input, and gives the predicted residual to the policy as an input. \emph{Advantage:} TRAIL avoids pixel-level reconstruction and needs no stage that maps latent actions to executable actions. \emph{Learning dynamics without latent actions:} the dynamics are supervised by the future prediction objective itself, because the predictor must produce the representation of a future frame from the current scene and the instruction.}
