...
and Table~III report the results on Meta-World MT10: ten tasks with distinct scenes, objects and motion primitives, covered by a \emph{single} language-conditioned policy trained by imitation learning. All the backbones were frozen including ours followed by the identical pooled action decoder, so that the representation supplied to the decoder is the only quantity that varies across rows. We report mean success over three random seeds with 50 closed-loop episodes per task and seed, using demonstration splits, seeds, camera and proprioceptive inputs and evaluation configurations that are shared across
methods. Note that Franka Kitchen results are now also reported based on the results from three random seeds and two viewpoints.
...
\R{Thanks for pointing out our mistake. We have modified the description about the hyper-parameters in Sec.~III-B and Sec.~IV-A.}

% $k$ is a frame gap used when sampling pre-training pairs from web video; it is \emph{not} a control horizon, and no robot ever
...
% the action time scale from the labelled demonstrations. Both points are now stated explicitly, in Sec.~IV-A where $k$ is introduced and in Sec.~III-B where deployment is described.}
...
\R{Thanks for your comment. We have clarified the notations, and Sec.~III-A now introduces the visual sequence as one class token $V_{\texttt{cls}}$ and $196$ patch tokens $V_{\texttt{patch}}$, and the instruction separately as the language-token sequence $L$. $V'_{\texttt{cls}}$, $V'_{\texttt{patch}}$ and $L'$ denote the same quantities after bidirectional cross-attention. We have also modified the equations (Eqs.~1--2), the figure (Fig.~2) and paragraphs in Sec.~III-A}
...
\R{Sorry for the confusion. The sentence was incorrect and we have now modified it. The \emph{intent} stream is retained after pre-training, and the \textit{full-information} stream is discarded. The full-information stream exists only to provide the stopped consistency target during pre-training. We have modified Sec.~III-A, Sec.~III-B and the Fig.~2 caption.}
...
% The bidirectional fusion block is drawn as two directed updates, language$\,\to\,$visual and visual$\,\to\,$language, rather than as an undifferentiated box. The two predictor instances carry a shared-weights marker so that it is visually obvious they are one
...
\R{We have modified equations (Eqs.~1--2) and redrawn Fig.~2 to present our pipeline more clear.
}
% The figure and the equations now describe the same object. The essential point, which the
...
\R{In our real-robot tabletop tasks, the policy is not asked to observe the workspace while an object is held, but first predicts both picking and placing poses at an initial state. 


Specifically, the action space is a \emph{pick-and-place primitive} rather than a low-level command: the tabletop read-out reshapes the patch tokens of $z_t$ and $\Delta_z$ into spatial maps and predicts
language-conditioned affordance maps. We have modified Sec.~III-B to explicitly explain our setting.}


% n ow states explicitly that one decision in
...
% to the observation pose, a new --- again unoccluded --- observation is taken, and the next primitive is predicted from it. The loop is therefore closed at the granularity of primitives, and every
