...
\begin{itemize}[leftmargin=1.4em,itemsep=2pt,topsep=4pt]
  \item \textbf{The asymmetric prediction mechanism is re-described.} The two streams are now presented as \emph{one} shared predictor $P$ with shared future queries $Q$, evaluated under two memory-access conditions (Eqs.~1--2), and the section closes with an explicit statement of where gradients stop and which modules are frozen versus trainable (Sec.~III-A).
  \item \textbf{The stream retained at deployment is corrected} to the \emph{intent} stream. Sec.~III-B now specifies the output structure of the retained predictor, the read-outs used by the two policy classes, and the closed-loop execution frequency.
  \item \textbf{A second simulation benchmark.} Meta-World MT10 is added (Sec.~IV-C, Table~III): ten heterogeneous tasks covered by a single language-conditioned policy trained by offline behavioral cloning. Both simulation benchmarks are now averaged over three random seeds, and the real-robot evaluation has been extended from 5 to 15 trials per subtask.
  \item \textbf{Experimental protocols are specified.} The revised experiments provide the applicable camera views, image resolution, proprioceptive inputs, frame stacking, depth usage, object randomization and separation between demonstration and evaluation seeds.
  \item \textbf{Relation to latent action models.} We have compared our method with latent action pre-training in Sec.~II-B.
  \item \textbf{Figures and wording.} We have substantially improved our pipeline figure and modified the caption to introduce the details more clearly.
  \item \textbf{Supplementary videos.} The robot manipulation videos are available in the supplementary video and on the anonymized project webpage (linked in the manuscript), and their playback speeds are now labeled.
\end{itemize}
...
We also include a version of the paper with changes from the IROS 2026 submission highlighted at the end of this document (additions in blue with a wavy underline, deletions in red and struck out).
...
\R{We thank the Associate Editor for identifying this ambiguity. The previous manuscript incorrectly stated which stream is retained and did not make the parameter sharing or gradient flow sufficiently explicit. In this submission, Sec.~III-A and Fig.~2 describe a \emph{single}
...
the consistency transfer. There is no gradient coupling through the text backbone as DistilBERT is frozen. Note that the current-frame visual encoder, language projection, fusion module, future queries and shared predictor are trainable. These details are now stated after Eqs.~(3)--(6) and visualized in Fig.~2.
...
latent action training. Prior works [1, 2] have utilized a latent action representation for policy
...
\R{Thanks for the suggestion. We revised Sec.~II-B by adding a discussion on latent action models and comparison with TRAIL. Both latent action models and TRAIL learn dynamics from action-free videos through future prediction. However, latent action models infer a low-capacity latent action from the current and future frames, use it to reconstruct the future frame conditioned on the current frame, and then pre-train policies to predict the latent action, which must later be mapped to executable actions using action-labeled data. TRAIL instead predicts future representations in latent space through a task-relevant bottleneck on the predictor's visual input, and gives the predicted residual to the policy as an input. Hence, TRAIL avoids pixel-level reconstruction and needs no stage that maps latent actions to executable actions.}
...
\R{We have conducted another experiment on a harder simulation benchmark that consists of 10 different robotic manipulation tasks. Sec.~IV-C
...
\R{Figure~2 has been redrawn with an explicit legend (frozen modules, shared weights, stop-gradient, bidirectional cross-attention, and the latent tokens), and the two directions of the bidirectional cross-attention are drawn as Language$\rightarrow$Visual and Visual$\rightarrow$Language. Every module is marked accordingly: the text backbone (DistilBERT) and the DINOv3 teacher are frozen, and all remaining modules are trainable during pre-training.}
...
\R{We have modified equations (Eqs.~1--2) and redrawn Fig.~2 to present our pipeline more clearly.}
...
over both, under the same frozen-backbone behavioral-cloning protocol as all baselines.
...
At the start of each trial, the arm is at its initial pose and the wrist-mounted camera sees the entire workspace; this single image (shown as ``Obs'') is the policy input, which is why it resembles an overhead view. We have added this description to Sec.~IV-D.}
...
\R{In our real-robot tabletop tasks, the policy is not asked to observe the workspace while an object is held, but first predicts both picking and placing poses at an initial state. We have added this description to Sec.~IV-D.


Specifically, the action space is a \emph{pick-and-place primitive} rather than a low-level command: the tabletop read-out reshapes the patch tokens of $z_t$ and $\Delta_z$ into spatial maps and predicts language-conditioned affordance maps. We have modified Sec.~III-B to explicitly explain our setting.}
...
\begin{itemize}[leftmargin=1.4em,itemsep=3pt,topsep=4pt]
  \item \emph{Simulation (Franka Kitchen, Meta-World).} The policy outputs low-level continuous
  ...
  \item \emph{Real robot.} The policy outputs a pick-and-place primitive. At the start of each trial, one observation is encoded, the intent stream runs once, and the resulting affordance prediction supplies both endpoints of the motion; the primitive then executes without re-observation. This is precisely why the occlusion concern in comment~6 does not arise: no observation is ever taken while an object is held.
\end{itemize}

In both cases every decision is made from a fresh observation, the predictor is
evaluated exactly once per decision, and the pre-training offset $k$ plays no role --- it does not
set the robot's action frequency, which the downstream decoder learns from the labeled
demonstrations. Fig.~1 (bottom) draws this loop.}

\item \C{The experimental protocol lacks important details on scene initialization, and only five
...
\emph{Scene initialization in simulation.} Object positions are randomized by the benchmark at
every reset, and the evaluation seeds control that randomization. Crucially, the evaluation seeds
...
\emph{Scene initialization on the real robot.} At the start of each trial the objects are placed by
hand at randomized positions and orientations within the $60\,\mathrm{cm}\times30\,\mathrm{cm}$
workspace, so no trial repeats a memorized layout. All four methods --- CLIPort, MPI, LaVA-Man and
...
attributable to the representation rather than to a more favorable draw of initial states.}
...
\R{We agree with the reviewer's distinction and have removed the R3M citation from this claim.}
...
% ------------------------------------------------------------------
% Paper with changes from the IROS 2026 submission highlighted (compiled from ieeeconf/diff_iros2026.tex).
\includepdf[pages=-,fitpaper]{diff_iros2026.pdf}
