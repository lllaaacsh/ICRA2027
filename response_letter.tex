...
\R{Thanks for the suggestion. We have added a paragraph at the end of Sec.~II-B, marked in
magenta, that compares TRAIL with latent action pre-training. Briefly, latent action models
constrain an \emph{action} variable and then need an alignment stage to map it onto real commands,
whereas TRAIL constrains \emph{visual memory access}, so $\Delta_z$ stays in the observation space
of a frozen encoder and requires no such stage. The dynamics are supervised by the future-latent
prediction objective itself, so $\Delta_z$ is the $k$-step transition that the instruction implies,
while the downstream policy is trained with real action labels. The trade-off is that $\Delta_z$ is
not an action and cannot be rolled out for planning.}
